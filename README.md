...
```
$ s2scfg=$(cat config.txt) && docker run -it --rm -e SPLATNET2STATINK_CONFIG=$s2scfg isseim/splatnet2statink:v1.5.8
```
...
```
$ docker run -it --name s2s-for-configuration --rm isseim/splatnet2statink:v1.5.8 -t
```
